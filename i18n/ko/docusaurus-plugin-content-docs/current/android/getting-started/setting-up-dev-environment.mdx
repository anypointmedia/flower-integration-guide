...
또한 SDK의 일부 전이 의존성(transitive dependency)은 Google Maven repository와 JitPack에서 받아오므로, AnypointMedia repository와 함께 `google()`과 `https://jitpack.io`도 반드시 선언해야 합니다.
...
:::caution
`google()` 또는 `https://jitpack.io`가 repository 목록에 없으면 SDK의 전이 의존성을 해석하는 단계에서 빌드가 실패합니다. 프로젝트가 `settings.gradle(.kts)`의 `dependencyResolutionManagement`에서 repository를 선언하는 구조라면, 모듈 수준 `build.gradle(.kts)` 대신 해당 위치에 추가하세요.
:::
...
**지원하는 `androidx.media3` 버전: 1.2.1 이상.** 사용 중인 `androidx.media3` 버전에 맞는 아티팩트를 선택하세요.

| 사용 중인 `androidx.media3` 버전 | 추가할 아티팩트 |
| --- | --- |
| 1.2.1 이상 | `flower-sdk:sdk-android-ott-media3-{androidx.media3-version}:X.X.X` |
...
:::info
위 표에 해당하지 않는 `androidx.media3` 버전(예: 1.2.1 미만)을 사용 중이라면 [dev-support@anypointmedia.com](mailto:dev-support@anypointmedia.com)으로 문의해 주세요.
:::
