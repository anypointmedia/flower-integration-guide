...
This prompt guides an LLM to install the Flower SDK package, apply the required native configuration, and initialize the SDK in a React Native project.

**Before using:** Replace `{{SDK_VERSION}}` and `{{PLAYER_TYPE}}`.
