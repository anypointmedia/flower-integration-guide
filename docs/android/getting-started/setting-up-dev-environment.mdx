...
The SDK also resolves part of its transitive dependencies from Google's Maven repository and JitPack, so `google()` and `https://jitpack.io` must be declared together with the AnypointMedia repository.
...
:::caution
If `google()` or `https://jitpack.io` is missing from the repository list, the build fails while resolving the SDK's transitive dependencies. If your project declares repositories in `settings.gradle(.kts)` under `dependencyResolutionManagement`, add them there instead of in the module-level `build.gradle(.kts)`.
:::
...
**Supported `androidx.media3` versions: 1.2.1 and above.** Choose the artifact that matches the `androidx.media3` version used by your app:

| Your `androidx.media3` version | Artifact to add |
| --- | --- |
| 1.2.1 and above | `flower-sdk:sdk-android-ott-media3-{androidx.media3-version}:X.X.X` |
...
:::info
If your project uses an `androidx.media3` version that is not covered by the artifacts listed above (for example, versions below 1.2.1), please contact us at [dev-support@anypointmedia.com](mailto:dev-support@anypointmedia.com).
:::
